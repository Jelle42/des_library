...
from des_library.Assignment3.statistics_helper import *
...
class CTDepartment:
    ...
    def _weekly_reset_schedule(self, sim: Simulation, event: Event):
            hour = event.time / 60 % 24
            self.week = int(event.time / (7*24*60))
            if not (self.day_of_week == 4, self._last_update_week != self.week and hour > 16): return
            # at end of friday, empty schedule and schedule all waiting outpatients
            self.reset_schedule()
            remaining_patients = []
